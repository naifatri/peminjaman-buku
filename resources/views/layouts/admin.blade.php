...
<body class="font-sans antialiased bg-slate-50" x-data="{ sidebarOpen: false }">
    <div class="app-shell flex min-h-dvh overflow-hidden lg:h-screen">
        ...
        <div x-show="sidebarOpen" class="fixed inset-0 z-40 bg-slate-900/60 lg:hidden backdrop-blur-sm" @click="sidebarOpen = false" x-transition.opacity x-cloak></div>

        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="app-sidebar fixed inset-y-0 left-0 z-50 w-72 max-w-[86vw] bg-[#0f172a] text-slate-300 transition-transform duration-300 ease-in-out lg:static lg:max-w-none lg:translate-x-0 flex flex-col shadow-2xl">
            <!-- Sidebar Header -->
            <div class="flex items-center px-5 sm:px-6 lg:px-8 h-20 bg-[#1e293b]/50 border-b border-slate-700/50">
                <div class="flex items-center space-x-3 group cursor-pointer">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[10px] uppercase tracking-[0.2em] text-slate-400 font-semibold leading-tight">Admin Console</span>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
                ...
                <a href="{{ route('admin.dashboard') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-chart-pie mr-4 text-lg {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Dashboard</span>
                </a>

                <a href="{{ route('admin.categories.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.categories.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-tags mr-4 text-lg {{ request()->routeIs('admin.categories.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Kategori</span>
                </a>

                <a href="{{ route('admin.books.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.books.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-book mr-4 text-lg {{ request()->routeIs('admin.books.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Buku</span>
                </a>

                <div class="px-4 mt-8 mb-2">
                    <span class="text-[11px] font-bold uppercase tracking-widest text-slate-500">Transaksi & Pengguna</span>
                </div>

                <a href="{{ route('admin.users.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.users.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-users mr-4 text-lg {{ request()->routeIs('admin.users.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Pengguna</span>
                </a>

                <a href="{{ route('admin.borrowings.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.borrowings.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-exchange-alt mr-4 text-lg {{ request()->routeIs('admin.borrowings.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Peminjaman</span>
                </a>

                <a href="{{ route('admin.fines.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.fines.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-file-invoice-dollar mr-4 text-lg {{ request()->routeIs('admin.fines.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Denda</span>
                </a>
                ...
                <div class="px-4 mt-8 mb-2">
                    <span class="text-[11px] font-bold uppercase tracking-widest text-slate-500">Laporan</span>
                </div>

                <a href="{{ route('admin.reports.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.reports.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-print mr-4 text-lg {{ request()->routeIs('admin.reports.*') ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400' }}"></i>
                    <span>Laporan</span>
                </a>
            </nav>
            ...
            <header class="app-header sticky top-0 z-10 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-6 lg:h-20 lg:flex-nowrap lg:px-8 lg:py-0">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0 lg:hidden">
                        ...
                        <p class="truncate text-base font-bold text-slate-800">@yield('header', 'Admin Dashboard')</p>
                    </div>
                            ...
                            <div class="max-h-96 overflow-y-auto custom-scrollbar">
                                @forelse(($headerNotifications ?? collect()) as $notification)
                                    <a href="{{ $notification->url }}" class="flex gap-3 px-5 py-4 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-b-0">
                                        <div class="mt-0.5 w-10 h-10 rounded-2xl flex items-center justify-center
                                            ...
                                            {{ $notification->accent === 'slate' ? 'bg-slate-100 text-slate-600' : '' }}">
                                ...
                                @empty
                                    <div class="px-5 py-10 text-center">
                                        ...
                                        <p class="text-sm text-slate-400 mt-3">Belum ada notifikasi untuk admin.</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                    ...
                    <div x-data="{ dropdownOpen: false }" class="relative">
                        <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-3 p-1.5 hover:bg-slate-50 rounded-xl transition-all duration-200 focus:outline-none group">
                            ...
                            <div class="hidden md:flex flex-col items-start leading-tight">
                                ...
                                <span class="text-[10px] font-medium text-slate-400 uppercase tracking-tighter">Administrator</span>
                            </div>
                ...
                <div class="app-page mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
                    ...
                    @if (session('success'))
                        <div class="mb-6 flex items-center p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-800 rounded-r-2xl shadow-sm" x-data="{ show: true }" x-show="show" x-transition>
                            ...
                            <button @click="show = false" class="ml-auto text-emerald-400 hover:text-emerald-600 focus:outline-none transition-colors"><i class="fas fa-times"></i></button>
                        </div>
                    @endif
                    @if (session('error'))
                        <div class="mb-6 flex items-center p-4 bg-rose-50 border-l-4 border-rose-500 text-rose-800 rounded-r-2xl shadow-sm" x-data="{ show: true }" x-show="show" x-transition>
                            ...
                            <button @click="show = false" class="ml-auto text-rose-400 hover:text-rose-600 focus:outline-none transition-colors"><i class="fas fa-times"></i></button>
                        </div>
                    @endif
